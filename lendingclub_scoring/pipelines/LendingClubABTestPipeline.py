...
FEATURES = ["term", "home_ownership", "purpose", "addr_state", "verification_status", "application_type", "loan_amnt",
            "emp_length", "annual_inc", "dti", "delinq_2yrs", "revol_util", "total_acc", "credit_length_in_years",
            "int_rate", "net", "issue_year"]


class LendingClubABTestPipeline():
    def __init__(self, spark, input_path, output_path, model_name, prod_version, test_version, limit=None):
        self.spark = spark
    ...
    def run(self):
        ...
        res_df = self.score_model(a_df, self.prod_version).union(self.score_model(b_df, self.test_version))

        res_df.write.format('delta').mode('overwrite').save(self.output_path)

    def score_model(self, df: DataFrame, version: str) -> DataFrame:
        model_uri = 'models:/{}/{}'.format(self.model_name, version)
        udf = mlflow.pyfunc.spark_udf(self.spark, model_uri)
        df = df \
            .withColumn("prediction", udf(*[col(c) for c in FEATURES])) \
